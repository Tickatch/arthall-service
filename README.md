...
> 
> vector 값은 픽셀(px) 기준이 아닌 **무대 영역을 기준으로 한 비율 좌표(normalized coordinate)** 로 저장되며,  
> 화면 크기나 해상도, 클라이언트 비율에 따라 좌석 위치 의미가 변하지 않도록 설계되었다.  
>  
> 프론트엔드는 자신의 렌더링 영역에 맞게 해당 좌표를 다시 픽셀 값으로 변환하여 좌석을 그리며,  
> 이를 통해 반응형 UI 환경에서도 동일한 좌석 배치를 유지할 수 있다.
