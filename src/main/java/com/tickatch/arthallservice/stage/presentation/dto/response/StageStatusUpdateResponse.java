...
public record StageStatusUpdateResponse(Long stageId, String name, StageStatus status) {

  public static StageStatusUpdateResponse from(StageResult result) {
    return new StageStatusUpdateResponse(result.stageId(), result.name(), result.status());
  }
}
